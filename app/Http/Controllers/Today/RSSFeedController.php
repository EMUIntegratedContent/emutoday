...
class RSSFeedController extends Controller{
	 ...
	 * @return string
	 */
	public function getEventsICalMinical($minical = null){
		// Get all future events and past events of two months ago or less
		if(!$minical){
			$events = Event::where([
				['is_approved', 1],
				['start_date', '>=', date('Y-m-d', strtotime(date("Y-m-d", strtotime("-2 months"))))]
			])
				->whereDoesntHave('minicalendars')
				->orderBy('start_date', 'asc')
				->get();
		}
		else {
			// Get the id of the minical based on the slug
			$mcal = MiniCalendar::where('slug', $minical)->first();
			if(!$mcal) {
				$events = [];
			} else {
				$m_id = $mcal->id;
				$events = Event::where([
					...
					->whereHas('minicalendars', function($query) use ($m_id){
						$query->where('mini_calendar_id', $m_id);
					})
					->with(['minicalendars' => function($query) {
