      ...
      <div class="large-9 medium-8 small-12 columns">
        <div id="issue-grouping" class="row">
          <div class="large-8 medium-8 small-12 columns">
            @if($magazine)
              <a href="/magazine/{{$magazine->year}}">
                <h2 class="issue-date news-caps">{{$magazine->season}} {{$magazine->year}}</h2>
              </a>
            @else
              <a href="#">
                <h2 class="issue-date news-caps">Season Year</h2>
              </a>
            @endif
          </div>
          <div class="large-4 medium-4 small-12 columns noleftpadding">
            <div class="addthis magazine-top">
              <a href=""><img src="/assets/imgs/icons/fake-sharethis.png" alt="addthis "></a>
            </div>
          </div>
        </div>
        <div class="row">
        ...
        <div id="story-content-edit">
          {!! html()->textarea('content', $story->content)->class('form-control')->id('cktextarea') !!}
        </div>
        ...
        <div class="featured-content-block magazine-block">
          ...
          <ul class="feature-list">
            @foreach ($sideStoryBlurbs as $ssblurb)
              <li><a href="#">{{ $ssblurb ? $ssblurb->caption : 'Missing article small caption' }}</a></li>
            @endforeach
          </ul>
        </div>
