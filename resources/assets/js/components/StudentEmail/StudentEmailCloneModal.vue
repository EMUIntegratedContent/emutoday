...
export default {
  ...
  methods: {
    cloneEmail: function() {
      this.$http.post('/api/student-email/clone', this.emailBuilderEmail)
      .then((response) =>{
        this.disableClone = true
        this.cloneStatus.is_set = true
        this.cloneStatus.message = response.data.message + ' <a href="/admin/studentemail/' + response.data.newdata.id + '/edit">Go now.</a>'
      }).catch(() => {
        this.disableClone = true
        this.cloneStatus.is_set = true
