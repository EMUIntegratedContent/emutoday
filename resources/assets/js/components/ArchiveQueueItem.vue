      ...
      <div class="box-header with-border">
        <div class="row">
          <a v-on:click.prevent="toggleBody" href="#">
            <div class="col-sm-12">
              <h6 class="box-title"><span class="arrowBuffer"><i :class="expandArrow"></i></span><span
                  class="badge">{{ item.story_type }}</span> {{ item.title }}</h6>
            </div>
            <!-- /.col-md-12 -->
          </a>
        </div>
      ...
      <div class="box-footer list-footer">
        <div class="row">
          <div class="col-sm-12 col-md-9">
            <p v-if="entityType == 'announcements'">Submitted on {{ formatDate(item.start_date) }} by
              {{ item.submitter }}</p>
            ...
            <div v-show="showArchivedButtons">
              ...
              <div class="btn-group pull-right">
                ...
                <div v-show="confirmDelete" class="btn-group pull-right">
                  ...
                  <button @click="deleteItem(item)" type="button" class="btn bg-red btn-xs footer-btn"
                          aria-label="delete item yes">YES
                  </button>
                </div>
              </div>
            </div>
            ...
            <div v-show="showUnarchivedButtons">
              ...
              <div class="btn-group pull-right">
                <a :href="editItem(item)" type="button" class="btn bg-orange btn-xs footer-btn"
                   aria-label="edit item"><i class="fa fa-pencil"></i></a>
              </div>
            </div>
            <!-- /.btn-toolbar -->
...
export default {
  ...
  methods: {
    ...
    unarchiveItem(item) {
      const url = '/api/archive/' + this.entityType + '/' + item.id + '/unarchive'
    ...
    deleteItem(item) {
      const url = '/api/archive/' + this.entityType + '/' + item.id + '/delete'
