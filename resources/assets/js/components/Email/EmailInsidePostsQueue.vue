        ...
        <div class="form-group">
          <label for="include-inside-chk">
            ...
            <input
                ...
                :true-value="1"
                :false-value="0"
                v-model="emailBuilderEmail.exclude_inside_posts"
            >
          </label>
        </div><!-- /input-group -->
        <template v-if="emailBuilderEmail.insidePosts.length">
          <Sortable
              :list="emailBuilderEmail.insidePosts"
              itemKey="postId"
              tag="div"
              :options="{}"
              @update="updateOrder"
          ...
          <template v-if="!loadingQueue">
            <template v-if="filteredPosts.length > 0">
              <email-inside-post-pod
                  ...
                  pod-type="postqueue"
                  :item="post"
                  :posts="emailBuilderEmail.insidePosts"
                  :key="'post-' + index"
...
export default {
  ...
  methods: {
    fetchAllRecords: function () {
      ...
      let routeurl = '/api/email/insideposts';
    ...
    updateOrder: function (post) {
      this.updateInsidePostsOrder({ newIndex: post.newIndex, oldIndex: post.oldIndex })
    }
  }
}
</script>
