      ...
      <div class="box-footer list-footer">
        <div class="row">
          <div class="col-sm-12 col-md-9 ">
            <span :class="timeFromNowStatus">Live {{ timefromNow }}</span> <span
...
export default {
  ...
  computed: {
    isAnnouncement: function () {
      if (this.emailBuilderEmail.announcements) {
        for (let i = 0; i < this.emailBuilderEmail.announcements.length; i++) {
          if (this.emailBuilderEmail.announcements[i].id == this.item.id) {
            this.checked = true
            return true
          }
        }
  ...
  methods: {
    ...
    toggleEmitAnnouncement: function (announcementObj) {
      ...
      if (!this.checked) {
        this.addAnnouncement(announcementObj)
      }
      else {
        this.removeAnnouncement(announcementObj.id)
      }
    },
